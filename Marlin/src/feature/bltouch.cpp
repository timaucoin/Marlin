...
void BLTouch::init(const bool set_voltage/*=false*/) {
  ...
  #else
  if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPAIR("last_written_mode - ", (int)last_written_mode);
  #if ENABLED(BLTOUCH_SET_5V_MODE)
    if (DEBUGGING(LEVELING)) DEBUG_ECHOLN("config mode - BLTOUCH_SET_5V_MODE");
  #else
    if (DEBUGGING(LEVELING)) DEBUG_ECHOLN("config mode - OD");
  #endif
